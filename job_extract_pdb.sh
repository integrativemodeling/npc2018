...
MODELING_SCRIPT=3_extract_pdb.py
SAXS_FILE=SAXS.dat
XL_FILE=XL.csv
RMF_FILE=../data_npc/WholeNPC_rmfs/47-35_1spoke.rmf3
RMF_FRAME=0
...
if [ $PWD_PARENT != $PWD ]; then
    ...
    echo "$IMP python ./$MODELING_SCRIPT -r $REPEAT -out $OUTPUT -rmf $RMF_FILE -rmf_n $RMF_FRAME"
    $IMP python ./$MODELING_SCRIPT -r $REPEAT -out $OUTPUT -rmf $RMF_FILE -rmf_n $RMF_FRAME
    cd ..
fi
